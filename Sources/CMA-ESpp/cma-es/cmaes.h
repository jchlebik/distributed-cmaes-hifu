...
public:
  ...
  void clean()
  {
    ...
    delete[] tempRandom; tempRandom = nullptr;
    delete[] BDz; BDz = nullptr;
    delete[] --xmean; xmean = nullptr;
    delete[] --xold; xold = nullptr;
    delete[] --xBestEver; xBestEver = nullptr;
    delete[] --output; output = nullptr;
    delete[] rgD; rgD = nullptr;
    for(int i = 0; i < params.N; ++i)
    {
      delete[] C[i]; C[i] = nullptr;
      delete[] B[i]; B[i] = nullptr;
    }
    for(int i = 0; i < params.lambda; ++i)
    {
      delete[] --(population[i]); population[i] = nullptr;
    }
    delete[] population; population = nullptr;
    delete[] C; C = nullptr;
    delete[] B; B = nullptr;
    delete[] index; index = nullptr;
    delete[] publicFitness; publicFitness = nullptr;
    delete[] --functionValues; functionValues = nullptr;
    delete[] --funcValueHistory; funcValueHistory = nullptr;
  }
   ...
   */
  T* init(const Parameters<T>& parameters, const long unsigned seed = 0, bool updateParams = true)
  {
    if(updateParams)
      params = parameters;
  ...
  T* const* samplePopulation()
  {
    ...
    { // generate scaled random vector D*z
      ...
      for(int i = 0; i < params.N; ++i)
        if(diag)
          rgrgxink[i] = std::fabs(xmean[i]) + sigma*rgD[i]*rand.gauss();
        else
          tempRandom[i] = rgD[i]*rand.gauss();
      if(!diag)
        for(int i = 0; i < params.N; ++i) // add mutation sigma*B*(D*z)
        {
          ...
          for(int j = 0; j < params.N; ++j)
            sum += B[i][j]*tempRandom[j];
          rgrgxink[i] = std::fabs(xmean[i]) + sigma*sum;
        }
    }
  ...
  void setSigma(T newSigma)
  {
    sigma = newSigma;
  }

  void setBestEver(T bestEverF, T* bestEverX, T fCalls)
  {
    for (int i = 0; i < params.N; i++)
    {
      xBestEver[i] = bestEverX[i];
    }
    xBestEver[params.N]   = bestEverF;
    xBestEver[params.N+1] = fCalls;
  }

  T* const* pseudoReset(T* const* pop, T* arFunVals)
  {
    T bestSoFar   = get(CMAES<T>::FBestEver);
    T currentGen  = get(CMAES<T>::Generation);
    T fitEvals    = get(CMAES<T>::Eval);

    T xBestEverCountEvals    = xBestEver[params.N+1];
    //T oldGenOfEigensysUpdate = genOfEigensysUpdate;

    T* mean        = getNew(CMAES<T>::XMean);
    T* bestChromo  = getNew(CMAES<T>::XBestEver);

    for (int i = 0; i < params.N; i++)
    {
      params.xstart[i] = std::fabs(mean[i]);
    }
    delete[] mean;

    clean();
    init(params, rand.getAktseed(), false);

    gen                 = currentGen + 1;
    countevals          = fitEvals;
    //genOfEigensysUpdate = oldGenOfEigensysUpdate;
    state               = INITIALIZED;
    
    for (int i = 0; i < params.N; i++)
    {
      xBestEver[i] = bestChromo[i];
    }
    xBestEver[params.N]   = funcValueHistory[0] = bestSoFar;
    xBestEver[params.N+1] = xBestEverCountEvals;
    delete[] bestChromo;

    for (int i = 0; i < params.lambda; i++)
    {
      functionValues[i] = arFunVals[i];
    }

    return population;
  }

  T* const* replaceIndividual(int oldIndvIndex, T* newIndv)
  {
    T* x;
    x = population[oldIndvIndex];
  ...
  T* const* reSampleGene(int popIndex, int geneIndex)
  {
    T* x;
    assert(popIndex >= 0 && popIndex < params.lambda &&
        "reSampleGene(): popIndex must be between 0 and sp.lambda");
    assert(geneIndex >= 0 && geneIndex < params.N &&
        "reSampleGene(): geneIndex must be between 0 and sp.N");
    x = population[popIndex];
    addMutationToGene(x, geneIndex);
    return population;
  }

  void addMutationToGene(T* x, int geneIndex, T eps = 1.0)
  {
    T sum = 0.0;
    for(int i = 0; i < params.N; ++i)
    {
      tempRandom[i] = rgD[i]*rand.gauss();
      sum += B[geneIndex][i]*tempRandom[i];
    }  
    x[geneIndex] = xmean[geneIndex] + eps*sigma*sum;
  }
  ...
  T* peekNextStepSize(const T* fitnessValues)
  {
    T _sigma = sigma;
    ...
    {
      _sigma *= std::exp(T(0.2) + params.cs / params.damps);
    }
    ...
    for(int i = 0; i < N; ++i)
    {
      ...
      for(int iNk = 0; iNk < params.mu; ++iNk)
        _xmean[i] += params.weights[iNk] * population[index[iNk]][i];
      _BDz[i] = sqrtmueffdivsigma * (_xmean[i] - _xold[i]);
    }

    T* _tempRandom = new T[N];
    ...
    for(int i = 0; i < N; ++i)
    {
      ...
      else
      {
        ...
        T* Bi = B[i];
        for(int j = 0; j < N; ++j)
          sum += Bi[j]*_tempRandom[j];
      }
      _ps[i] = invps*ps[i] + sqrtFactor*sum;
    }
    ...
    // update of sigma
    _sigma *= std::exp(((std::sqrt(psxps) / chiN) - T(1))* params.cs / params.damps);
